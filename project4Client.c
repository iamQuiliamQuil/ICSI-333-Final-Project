...
void buildACLientSocket (int * talkSocket, char **argv){
  struct addrinfo hints, *res, *ptr;

  //sets everything int hints to 0, sets some useful defaults and prevents segmentation faults
  memset(&hints, 0, sizeof(hints));
  hints.ai_family = AF_UNSPEC; //IPV4 &IPV6 work
  hints.ai_socktype = SOCK_STREAM; //TCP
  getaddrinfo(argv[2], argv[1], &hints, &res); //passes port & IP adress from command line

  for (ptr = res;ptr != NULL; ptr=ptr->ai_next) {
    //navigates linked list containg addrinfo & pointed to by res
    if ((*talkSocket = socket(ptr->ai_family, ptr->ai_socktype, ptr->ai_protocol))<0) {
      continue;
      //if socket fails, try it on the next node in the linked list
    }
    if (connect(*talkSocket, ptr->ai_addr, ptr->ai_addrlen) < 0) {
      //if connect fails, try it on the next node in the linked list, also close the socket you just made
      perror("Connect failed");
      close(*talkSocket);
      continue;
    }
    //you only get this far if socket & connect fail
    break;
  }
  if (ptr == NULL) //if ptr == NULL, no node was capable of being a working socket
    exit(1);

  //don't need this anymore, let my heap go
  freeaddrinfo(res);

}

void killAClientSocket (int * talkSocket){
  //closes the socket made in buildACLientSocket(z)
  if (close(*talkSocket)==-1){
...
struct World * initialize(int * talkSocket, char **argv)
{
    buildACLientSocket(talkSocket, argv);
...
void teardown(struct World *world, struct Node *headPtr, int talkSocket)
{
    free(world);
    world = NULL;
    ...
    //writes to a log file and dismanstles linked list of logged info
    printAndFree(headPtr, f);;
    fclose(f);

    killAClientSocket(&talkSocket);
}
...
void displayWorld(struct World *world){
    ...
    for(int j = 0; j < 10; j++)
    {
        ...
        for(int i = 0; i<10; i++){
            //printf("%i|",(*world).coord[i][j]); - for debugging
...
int main(int argc, char const *argv[]) {
  if (argc < 3){
    puts("Too few arguments; input a Port and an IP Adress");
    return -1;
  }
    ...
    while (1){
      ...
      //flag = 0  by default, will change when player has won/lost
      if (flag){
        if (flag == WIN){
          puts("You win!");
          break;
    ...
    puts("Gets to teardown");
    teardown(world, headPtr, talkSocket);
